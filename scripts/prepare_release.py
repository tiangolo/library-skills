"""Prepare a release by updating package versions and release notes."""
...
def update_json_version_file(content: str, version: str, version_file: Path) -> str:
...
def update_package_lock_file(content: str, version: str, version_file: Path) -> str:
    ...
    if not isinstance(packages, dict):
        raise RuntimeError(f"Could not find packages in {version_file}")
    root_package = packages.get("")
    if not isinstance(root_package, dict):
        raise RuntimeError(f"Could not find root package in {version_file}")
...
def prepare(
    ...
    package_json_file: Annotated[
        Path,
        typer.Option(
            envvar="PREPARE_RELEASE_PACKAGE_JSON_FILE",
            exists=True,
            file_okay=True,
            dir_okay=False,
            readable=True,
            writable=True,
            help="Path to package.json containing the TypeScript package version.",
        ),
    ],
    package_lock_file: Annotated[
        Path,
        typer.Option(
            envvar="PREPARE_RELEASE_PACKAGE_LOCK_FILE",
            exists=True,
            file_okay=True,
            dir_okay=False,
            readable=True,
            writable=True,
            help="Path to package-lock.json containing the TypeScript package version.",
        ),
    ],
    ...
    package_json_content = package_json_file.read_text(encoding="utf-8")
    package_lock_content = package_lock_file.read_text(encoding="utf-8")
    ...
    package_json_file.write_text(
        update_json_version_file(package_json_content, version, package_json_file),
        encoding="utf-8",
    )
    package_lock_file.write_text(
        update_package_lock_file(package_lock_content, version, package_lock_file),
        encoding="utf-8",
    )
